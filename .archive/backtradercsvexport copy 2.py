...
STRATEGIES = retall()
STRATEGIES = load_strats()
CSV_PATH = "results.csv"

CSV_PATH = load_output_csv()
SYMBOLS = load_symbols()

filter_available(SYMBOLS)
...
def make_feed(symbol: str) :#-> bt.feeds.GenericCSVData | bt.feeds.PandasData:
    """
    1⃣  Look for a CSV in data/alpha or data/stooq and load it with GenericCSVData
        • daily files  : <SYM>[_adj].csv  (Date YYYY-MM-DD)
        • intraday     : <SYM>_<N>m.csv   (timestamp YYYY-MM-DD HH:MM)
    2⃣  If no local file exists, fall back to yfinance 1-hour bars (old behaviour).
    """
    # -------- 1. try local CSVs ------------------------------------------------
    for root in DATA_DIRS:
        # alpha: SPY_adj.csv  • stooq: SPY_d.csv  • intraday: SPY_5m.csv
        for fname in root.glob(f"{symbol}*.csv"):
            tz = "%Y-%m-%d %H:%M:%S" if "_m" in fname.stem else "%Y-%m-%d"
